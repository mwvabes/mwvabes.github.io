...
var academics = ["wbisiaArchitektura",
               "wbisiaBudownictwo",
...
function colourScores(courseccc) {
    ...
    for(j=4;j<scores.length;j++){
        if(score==0||isNaN(score)){courseForColour.childNodes[j].className=""
        }
...
function langBEh(){
    let ret;
    if(langBasic.value>(1.5*langExt.value)){ret=parseFloat(langBasic.value)}else{ret=parseFloat(1.5*langExt.value)}
...
function wbisiaArchitektura(){
    ...
    let output=out.childNodes[3];let scores=plBB()+langBEh()+mathBE()+histE();
    if(scores==0)scores="--";
...
var filledStatusPl=!1;
var filledStatusMath=!1;
var filledStatusLang=!1;
...
setTimeout(function(){score();colours()},100)}
...
function unFilled(input){
    ...
    if(input.value==""){
        ...
        if(input.id=="plBasic")filledStatusPl=!0;
        if(input.id=="mathBasic")filledStatusMath=!0;
        if(input.id=="langBasic")filledStatusLang=!0;
        console.log(filledStatusPl,filledStatusMath,filledStatusLang)}
        else if(input.value!=""){
            if(input.id=="plBasic")filledStatusPl=!1;
            if(input.id=="mathBasic")filledStatusMath=!1;
            if(input.id=="langBasic")filledStatusLang=!1;
...
courseTable.addEventListener("keyup",function(){
    check()},!1);
plBasic.addEventListener("blur",function(){unFilled(plBasic)},!1);
mathBasic.addEventListener("blur",function(){unFilled(mathBasic)},!1);
langBasic.addEventListener("blur",function(){unFilled(langBasic)},!1);
clearFormInput.addEventListener("click",function(){
    filledStatusReset();setTimeout(function(){score()},100)
},!1);
document.addEventListener('DOMContentLoaded',function(){
    courseTable.addEventListener("blur",function(){score()},!1);score();colours()
},!1)